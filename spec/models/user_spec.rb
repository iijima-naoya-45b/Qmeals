...
RSpec.describe User, type: :model do
  describe 'バリデーションチェック' do
    ...
    it 'name無しの場合、invalidになるか' do
      ...
      expect(user_without_name.errors[:name]).to eq %w[入力してください 3文字以上で入力してください]
    end
    ...
    it 'password_confirmation無しの場合、invalidになるか' do
      ...
      expect(user_without_password_confirmation.errors[:password_confirmation]).to eq %w[パスワードとパスワード確認が違います 入力してください]
    end

    it 'email無しの場合、invalidになるか' do
      user_without_email = build(:user, email: '')
      expect(user_without_email).to be_invalid
      expect(user_without_email.errors[:email]).to eq %w[入力してください このメールアドレスは使用できません]
    end
  end
end
